...
public class SaveMap : MonoBehaviour
{
    ...
    public bool test = false;//只有需要进行自动测试才为true！不需要的时候请关掉
    ...
    {
        frame_num++;
        if(frame_num%50==0&&test){
            OnClick_Test_Save_Load_Map();
        }
    }
    ...
    public void OnClick_Save_Map()
    {
        ...
        print(js);
        System.IO.File.WriteAllText(@".\Assets\Data\Mymap\" + id.ToString()+"_"+id.ToString()+ ".json", js);
